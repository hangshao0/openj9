...
JVM_CreateThreadSnapshot(JNIEnv *env, jobject thread)
{
	...
	if (NULL == thread) {
		...
		if (JVMTI_ERROR_NONE != vmfns->getTargetVMThreadHelper(currentThread, thread, JVMTI_ERROR_NONE, 0, &targetThread, &isVirtual, &isAlive)) {
			goto done;
		}
		...
		resultObject = vm->memoryManagerFunctions->J9AllocateObject(currentThread, J9VMJDKINTERNALVMTHREADSNAPSHOT_OR_NULL(vm), J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
		...
		if (isVirtual) {
			...
			J9VMJDKINTERNALVMTHREADSNAPSHOT_SET_CARRIERTHREAD(currentThread, resultObject, J9VMJAVALANGVIRTUALTHREAD_CARRIERTHREAD(currentThread, threadObject));
			J9VMJDKINTERNALVMTHREADSNAPSHOT_SET_BLOCKEROBJECT(currentThread, resultObject,  J9VMJDKINTERNALVMCONTINUATION_BLOCKER(currentThread, continuationObject));
			/* blockerTypeOrdinal to be filled */
			/* status to be filled */
			/* stacktrace to be filled */
			/* locks to be filled */
		} else {
			J9VMJDKINTERNALVMTHREADSNAPSHOT_SET_BLOCKEROBJECT(currentThread, resultObject, vmfns->j9jni_createLocalRef(env, targetThread->blockingEnterObject));
			/* blockerTypeOrdinal to be filled */
			/* status to be filled */
			/* stacktrace to be filled */
			/* locks to be filled */
