...
restart:
				...
				if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_MONITOR_CONTENDED_ENTERED)
					&& J9_ARE_NO_BITS_SET(currentThread->currentContinuation->runtimeFlags, J9VM_CONTINUATION_RUNTIMEFLAG_JVMTI_CONTENDED_MONITOR_ENTER_RECORDED)
				) {
					/* It could be too late in yieldPinnedContinuation() that we cannot get the owner. Get the owner here. */
					if ((void*)1 == monitor->owner) {
						currentThread->currentContinuation->previousOwner = NULL;
					} else {
						currentThread->currentContinuation->previousOwner = getVMThreadFromOMRThread(vm, ((J9ThreadMonitor *)monitor)->owner);
					}
				}
