...
restart:
				...
				if (0 == omrthread_monitor_try_enter(monitor)) {
					...
					while (!J9_LOCK_IS_INFLATED(lock)) {
						...
						j9objectmonitor_t newLockword = 0;
						UDATA count = J9_FLATLOCK_COUNT(lock);
						if (0 == count) {
							/* Lock is unlocked, so try to directly acquire it as a flatlock. */
							newLockword = (j9objectmonitor_t)(UDATA)currentThread;
						} else {
							/* Change the lock to flat with FLC bit set so it will be inflated during exit. */
							newLockword = ((j9objectmonitor_t)J9_FLATLOCK_OWNER(lock)
										| ((j9objectmonitor_t)(count - 1) << OBJECT_HEADER_LOCK_V2_RECURSION_OFFSET)
										| OBJECT_HEADER_LOCK_FLC);
						}
						j9objectmonitor_t const oldValue = lock;
						lock = VM_ObjectMonitor::compareAndSwapLockword(currentThread, lwEA, lock, newLockword);
						if (lock == oldValue) {
					...
					if (!J9_LOCK_IS_INFLATED(lock)
					&& (OBJECT_HEADER_LOCK_FLC != (lock & (OBJECT_HEADER_LOCK_FLC | OBJECT_HEADER_LOCK_INFLATED)))
					) {
						goto restart;
					}
				}
