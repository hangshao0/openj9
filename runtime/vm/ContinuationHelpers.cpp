...
enterContinuation(J9VMThread *currentThread, j9object_t continuationObject)
{
	...
	printf("> mount %p on %p ownedMonitorCount %d, enteredMonitors %p %p, returnstate %d\n", continuation, currentThread, (int)currentThread->ownedMonitorCount, continuation->enteredMonitors, (continuation->enteredMonitors != NULL) ? continuation->enteredMonitors->next : NULL, (int)continuation->returnState);
...
yieldContinuation(J9VMThread *currentThread, BOOLEAN isFinished, UDATA returnState)
{
	...
	printf("< unmount %p from %p, enteredMonitors %p %p returnState %d\n", continuation, currentThread, continuation->enteredMonitors, (continuation->enteredMonitors != NULL) ? continuation->enteredMonitors->next : NULL, (int)returnState);
